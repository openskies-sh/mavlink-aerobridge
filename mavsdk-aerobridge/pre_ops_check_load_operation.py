    ...
    if aircraft_active_check:
        ...
        if aerobridge_firmware.status_code == 200:
            ...
            if firmware_details['version'] == firmware['flight_sw_version']:
                ...
                print("Firmware version mismatch {board_firmware} on the drone vs {aerobridge_firmware} on Aerobridge, drone cannot be armed".format(firmware_dict))
        else: 
    ...
    if aircraft_active_check:    
        ...
        if operation_permission.status_code == 200:
            ...
            if permission_data['status_code'] == "granted":
                ...
                print("Cannot arm drone, permission status %s" % permission_data['status_code'])
    mission_plan_data = False
    if permission_granted:
        # Flight permission has been granted downloading mission plan 
        # All checks passed, download flight operation permission and public key    
...
if __name__ == '__main__':
    ...
    if not operation_id:
        print("A valid Operation ID (UUID) from your Aerobridge instance must be provided before arming the drone with a mission" )
        exit()
